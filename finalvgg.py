...
model_conv.classifier = nn.Sequential(nn.Dropout(0.5), nn.Linear(num_ftrs,len(class_names)))
...
optimizer_conv = optim.SGD(model_conv.parameters(), lr=0.01, weight_decay=0, momentum=0.9)
...
df.to_csv("final_vgg.csv", index=False)
